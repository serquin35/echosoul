const fs = require('fs');
const data = JSON.parse(fs.readFileSync('C:/Users/Serquin/.gemini/antigravity/brain/50e98ced-f22b-4126-82a5-be745c50e460/.system_generated/steps/1346/output.txt', 'utf8'));
const wf = data.data;

const node = wf.nodes.find(n => n.name === 'Build step content');

node.parameters.jsCode = `const onb  = $('Loop users').item.json;
const prof = $input.first().json;

// Prevent crash if profile wasn't found
if (!prof || !prof.display_name) {
  return [{ json: {
    user_id: onb.user_id, email: '',
    fcm_token: '', platform: onb.platform,
...
fs.writeFileSync('updated_workflow.json', JSON.stringify(wf, null, 2));
console.log('Saved to updated_workflow.json');
